...
@section('main')
<style>
    /* Add the following styles to your existing stylesheet or in a <style> tag in the head section */
    .table-fixed {
        table-layout: fixed;
        width: 100%;
    }
                ...
                <table class="table table-bordered table-hover" id="dataTable" width="100%" cellspacing="none">
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td>{{ $info->reply }}</td>
                            <td> <a href="/Feedbackedit/{{ $info->id }}" class="btn btn-primary btn-sm text-decoration-none" style="border-radius:5px">Edit</a> |
                            <form onsubmit="return confirmdeleted(event)" action="/Feedbackdeleted/{{$info->id}}" method="post" class="d-inline">
                            @csrf
                            @method('DELETE')
...
<script>
    function confirmdeleted(event)
    {
        ...
        Swal.fire({
            ...
            text: "Deleted feedback cannot be return back",
            icon: 'warning',
