    ...
    @foreach($announcements as $announcement)
        <div class="card mb-3">
            <div class="card-body">
                ...
                <p class="card-text">
                    <small class="text-muted">Published on {{ $announcement->published_at->format('F j, Y') }}</small>
                </p>
                <div class="d-inline">
                <a href="/Announcementedit/{{$announcement->id}}" class="btn btn-warning text-decoration-none">Edit</a>
                <form onsubmit="return confirmdeleted(event)" action="/announcementsdeleted/{{ $announcement->id }}" method="POST" class="d-inline">
                    @csrf
                    @method('DELETE')
...
<script>
    function confirmdeleted(event)
    {
        ...
        Swal.fire({
            title: 'Confirm deteled your data?',
            text: "Deleted announcement cannot be return back",
            icon: 'warning',
